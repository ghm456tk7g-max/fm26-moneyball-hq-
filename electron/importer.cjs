...
const aliases = {
  ...
  apps: ['apps', 'apps (sub)', 'appearances', 'appearances (sub)', 'einsätze', 'einsatze', 'spiele'],
  minutes: ['minutes', 'mins', 'minuten', 'min', 'minutes played', 'gespielte minuten'],
  goals: ['goals', 'gls', 'tore'],
  assists: ['assists', 'ast', 'vorlagen'],
  rating: ['av rat', 'avg rating', 'average rating', 'average rating (overall)', 'durchschnittsbewertung', 'bewertung', 'note', 'ø-note'],
  ...
  contractEnd: ['contract expires', 'contract expiry', 'contract end', 'expires', 'vertrag bis', 'vertragsende']
};
...
function parseSingleMoney(value) {
  ...
  text = text
    .replace(/[€£$¥]/g, '')
    .replace(/(?:P\/?W|PER\s*WEEK|\/\s*WEEK|WEEKLY|P\.A\.|PER\s*ANNUM|\/\s*YEAR|ANNUAL)$/i, '')
    .replace(/[\s\u00A0\u202F]/g, '');
  let multiplier = 1;
...
function parseMoney(value) {
  ...
  if (!text || /^(?:-|—|n\/?a|unknown|not available|nicht verfügbar)$/i.test(text)) return null;

  // FM can export uncertain market values as ranges. The upper bound keeps
  // affordability decisions conservative.
...
function playerIdentityKey(player) {
  ...
  const club = normalizeIdentityPart(player.club);
...
function parseAppearances(value) {
  const combined = String(value ?? '').trim().match(/^(\d+)\s*\((\d+)\)$/);
  if (combined) return Number(combined[1]) + Number(combined[2]);
  return validNumber(value, { min: 0, max: 200 });
}

function completeness(player) {
  return ['club', 'position', 'age', 'apps', 'minutes', 'goals', 'assists', 'rating', 'value', 'wage', 'contractEnd']
    .filter(key => player[key] !== null && player[key] !== '').length;
}
...
function deduplicatePlayers(players) {
  ...
  for (const player of players) {
    player.identityKey = playerIdentityKey(player);
    const existing = unique.get(player.identityKey);
    if (!existing) {
      unique.set(player.identityKey, player);
      continue;
    }
    duplicateCount += 1;
    if (completeness(player) > completeness(existing)) unique.set(player.identityKey, player);
  }
...
function importFile(filePath, datasetType = 'targets') {
  ...
  const result = Papa.parse(decoded.text, {
    header: true,
  ...
  const map = {};
  Object.keys(aliases).forEach(key => { map[key] = findColumn(headers, key); });
  ...
  const parsedPlayers = result.data.map((row, index) => {
    ...
    const numeric = (key, bounds) => {
      if (!map[key]) return null;
      const raw = rawValue(key);
      const parsed = validNumber(raw, bounds);
      if (String(raw ?? '').trim() && parsed === null) invalidValueCount += 1;
      return parsed;
    };
    const monetary = key => {
      if (!map[key]) return null;
    ...
    const appearances = () => {
      if (!map.apps) return null;
      const raw = rawValue('apps');
      const parsed = parseAppearances(raw);
      if (String(raw ?? '').trim() && parsed === null) invalidValueCount += 1;
      return parsed;
    };
    return {
      ...
      club: map.club ? String(rawValue('club') || '').trim() : '',
      position: map.position ? String(rawValue('position') || '').trim() : '',
      age: numeric('age', { min: 14, max: 60 }),
      apps: appearances(),
      minutes: numeric('minutes', { min: 0, max: 20000 }),
      goals: numeric('goals', { min: 0, max: 500 }),
      assists: numeric('assists', { min: 0, max: 500 }),
      rating: numeric('rating', { min: 1, max: 10 }),
      value: monetary('value'),
      wage: monetary('wage'),
      contractEnd: map.contractEnd ? String(rawValue('contractEnd') || '').trim() : ''
    };
  ...
  return {
    ...
    map,
    warnings,
    rowCount: deduplicated.players.length,
